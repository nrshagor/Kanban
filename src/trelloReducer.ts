...
type ActionType = 
  ...
  | {type: "EDIT_TASK", payload: {listID: number, newtask: string,taskID: number}}
...
export default function trelloReducer(state: IState, action: ActionType) {
  switch(action.type) {
    ...
    case 'EDIT_TASK':
      const taskIdx = state.lists.findIndex(task => task.id === action.payload.taskID)
      const newTask = [...state.lists]
      newTask[taskIdx].name = action.payload.newtask 

      return {...state, lists: newTask}
